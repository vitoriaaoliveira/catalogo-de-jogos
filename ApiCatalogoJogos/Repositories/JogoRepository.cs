...
namespace ApiCatalogoJogos.Repositories
{
    ...
    public Task<List<Jogo>> Obter(int pagina, int quantidade, object jogos)
    {
        return Task.FromResult(jogos.Values.Skip((pagina - 1) * quantidade).Take(quantidade).ToList());
    }

    public Task<Jogo> Obter (Guid id, object jogos)
    {
    ...
    public Task<List<Jogo>> Obter(string nome, string produtora, object jogos)
    {
        return Task.FromResult(jogos.Values.Where(jogo => jogo.Nome.Equals(nome) && jogo.Produtora.Equals(produtora)).ToList());
    }

    public Task<List<Jogo>> ObterSemLambda(string nome, string produtora, object jogo)
    {
        var retorno = new  List<Jogo>();

        foreach (var Jogo  in  Jogos.Valores )
            {
